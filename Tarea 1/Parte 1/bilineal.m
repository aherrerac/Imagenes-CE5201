function Y = bilineal(A)
    ...
    for y = 2:n-1
        %Comparacion de pixeles negros
        if or(A(x,y,1)<=0.1,A(x,y,2)<=0.1,A(x,y,3)<=0.1)
            %Se promedian los 2 pixeles adyacentes, x = 1 
            Y(x,y,1:3) = (A(x+1,y-1,1:3) + A(x+1,y+1,1:3))/2;
        else
            Y(x,y,:) = A(x,y,:);
        endif
    endfor
    for y = 2:n-1
        if or(A(m,y,1)<=0.1,A(m,y,2)<=0.1,A(m,y,3)<=0.1)
            %x = m 
            Y(m,y,1:3) = (A(m-1,y-1,1:3) + A(m-1,y+1,1:3))/2;
        else
            Y(m,y,:) = A(m,y,:);
        endif
    endfor
    ...
    for x = 2:m-1
        %Comparacion de pixeles negros   
        if or(A(x,y,1)<=0.1,A(x,y,2)<=0.1,A(x,y,3)<=0.1)
            %Se promedian los 2 pixeles adyacentes, y = 1;
            Y(x,y,1:3) = (A(x-1,y+1,1:3) + A(x+1,y+1,1:3))/2;
        else
            Y(x,y,:) = A(x,y,:);
        endif 
    ...
    for x = 2:m-1
        if or(A(x,y,1)<=0.1,A(x,y,2)<=0.1,A(x,y,3)<=0.1)
            Y(x,n,1:3) = (A(x-1,n-1,1:3) + A(x-1,n-1,1:3))/2;
        else
            Y(x,n,:) = A(x,n,:);
        endif
    endfor
    %Se realiza el calculo del promedio de los 4 pixeles adyacentes
    for x = 2:m-1
        for y = 2:n-1
            if or(A(x,y,1)<=0.1,A(x,y,2)<=0.1,A(x,y,3)<=0.1)
                Y(x,y,1:3) = (A(x-1,y-1,1:3) + A(x+1,y-1,1:3) + A(x-1,y+1,1:3)  + A(x+1,y+1,1:3) )/4;
            else
                Y(x,y,:) = A(x,y,:);
            endif
        endfor
